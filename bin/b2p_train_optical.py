...
'''
This script trains all the RESNET models on 70% of the data from Uganda and Rwanda, exports the saved state of each RESNET model to a pkl file, 
and then evaluates all the RESNET models on the remaining 30% of the data from Uganda and Rwanda. 
It then prints out a report summarizing how each of the RESNET models performed on the held-out validation data (the remaining 30% of the data).  
This requires that someone decides which model they want to use for inference by-hand, but it allows for greater flexibility and provides all the 
possible models so that the models can be swapped out if the need arises.   
This function takes two arguments: a dictionary of the form {'Region Name': region_csv} where Region Name is a string like 'Uganda' or 'Rwanda' 
and region_csv is a cleaned version of the csv files provided by Kyle and a dictionary of the form {'Region Name': path_to_tiff_tiles} where 
Region Name is a string like 'Uganda' or 'Rwanda' and path_to_tiff_files is a path from the current working directory to the directory containing 
the tiff tiles for the current region.  This code assumes that we have already run the api_download.py script on the bounding boxes for the 
regions in Uganda and Rwanda to obtain all the S2 data for these regions, that we have then created a composite for the S2 data in these regions, 
and that we then called the make_tile_tiffs.py script to create 300m x 300m tiff tiles across all the S2 tiles in these regions.  
The path to the directory containing the 300m x 300m tiff tiles for each region should be placed in the path_to_tiff_tiles place in the 
aforementioned dictionary. 
'''

def train_model(csv_files: dict, tiff_dirs: dict):
    '''This function trains all the possible RESNET models and evaluates the performance of each of the possible RESNET models on validation data. 
    The reports of the model performance on the validation data are printed for each of the possible RESNET models.
    
    Arguments:
        csv_files: a dictionary whose keys are the names of the regions for which training data exists (i.e. Uganda, Rwanda) and whose values are the csv files of verified bridge locations in each region (see the csv files from Kyle)
        tiff_dirs: a dictionary whose keys are the names of the regions for which training data exists (i.e. Uganda, Rwanda) and whose values are the names of the directory containing the training tiff tiles for each region
    '''
    ...
    for region in regions:
        ...
        for tiff in os.listdir(train_dir):
            ...
            for geom in train_geometry:
                ...
                tiff_bound = Polygon(((tiff_bound[0], tiff_bound[1]), (tiff_bound[0], tiff_bound[3]), (tiff_bound[2], tiff_bound[3]), (tiff_bound[2], tiff_bound[1])))
    ...
    df = pd.DataFrame({'Region': region_name, 'Image Path': file_names, 'Label': labels})
    ...
    for lyr in [18, 34, 50, 101, 152]:
        ...
        f_score = partial(fbeta, thresh=0.2, beta=1, sigmoid=False)
        learn = vision_learner(data, arch, metrics = error_rate, ps = 0.75, bn_final=True)
        ...
        # save the trained and fine-tuned version of the current RESNET model
        diff_models.append(learn) 
        order.append(lyr)

        # evaluate the performance of the trained RESNET model
        interp = ClassificationInterpretation.from_learner(learn)
        interp.plot_confusion_matrix(normalize=True, cmap='jet_r')
        interp.plot_confusion_matrix(normalize=False, cmap='jet_r')
    ...
    # create a directory that will store the fine-tuned states of each of the trained RESNET models
    model_dir = os.get_cwd() + '/trained_models'
    os.mkdir(model_dir)

    # iterate over the saved states of all the RESNET models, define the name of each model, and write its current state to a pkl file
    for i in range(len(diff_models)):
        model_name = f'resnet{lyr[i]}_300px_r0-2500_g0-2500_b0-2500_v{date.today()}.pkl'
        diff_models[i].export(model_dir + '/' + model_name)
