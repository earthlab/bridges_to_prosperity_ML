...
if __name__ == "__main__":
    ...
    composite_parser.add_argument('--district', '-d', type=str, required=False,
                                  help='Name of the district to upload the composites for. Default is all districts '
                                       'per input region.')
    ...
    inference_parser.add_argument('--layers', required=False, nargs='+',
                                  help='If specified, only files made from these layers will be uploaded,'
                                       ' otherwise all layer combinations will be found')
    inference_parser.add_argument('--epoch', required=False, type=int,
                                  help='If specified, only files from this epoch will be uploaded,'
                                       ' otherwise all epochs will be found')
    ...
    inference_parser.add_argument('--tile_size', required=False, type=int,
                                  help='If specified only files of this tile size will be uploaded, '
                                       'otherwise all tile sizes will be found')
    ...
    if args.file_type == 'composites':
        ...
        files_to_download = TrainedModel.filter_files(s3_files, args.regions, args.architecture, args.layers,
                                                      args.epoch, args.ratio, args.tile_size, args.best)
    elif args.inference_results == 'inference_results':
        s3_files = list_s3_files(InferenceResultsCSV.ROOT_S3_DIR, args.s3_bucket_name)
        csv_files = InferenceResultsCSV.filter_files(s3_files, args.regions, args.architecture, args.layers,
                                                     args.epoch, args.ration, args.tile_size, args.best)
        tar_files = InferenceResultsTarfile.filter_files(s3_files, args.regions, args.architecture, args.layers,
                                                         args.epoch, args.ratio, args.tile_size, args.best)
    ...
    parallel_inputs = []
    for i, s3_file_group in enumerate(np.array_split(files_to_download, args.cores)):
        group = []
        for file in s3_file_group:
            file_class = File.create(file)
            file_class.create_archive_dir()
            group.append((file, file_class.archive_path, str(i + 1), args.s3_bucket_name))
        parallel_inputs.append(group)
    process_map(
        s3_download_task,
        parallel_inputs,
