...
from file_types import InferenceResultsCSV, TrainedModel
...
def run_inference(model_file_path: str, tile_csv_path: str, truth_data: bool,
    ...
    if model_file is None:
        raise ValueError(f'{os.path.basename(model_file_path)} not a valid model file')
    ...
    if os.path.exists(results_csv.archive_path()):
    ...
    Path(os.path.dirname(results_csv.archive_path())).mkdir(parents=True, exist_ok=True)
    ...
    inference_torch(
        ...
        results_csv_path=results_csv.archive_path(),
