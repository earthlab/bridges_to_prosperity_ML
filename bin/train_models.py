...
def train_models(regions: List[str], training_ratio: float, layers: List[str], tile_size: int, architectures: List[str] = ARCHITECTURES,
                 no_bridge_to_bridge_ratios: Union[None, List[float]] = None) -> None:
    """
    Trains models to infer bridge locations and outputs the models to archive. The architecture, no bridge / bridge ratio, and layers used to
    train the model can be specified.
    Args:
        ...
        layers (list): The names of the layers to use from the multivariate composites to train the model. Can pick up to 3 layers
          (red blue green nir osm-water osm-boundary elevation slope)
        tile_size (int): Size of the tiles in meters to use to train the models. These tiles and the corresponding tile match file should already 
          exist for each input region. 
        architectures (list): List of model architectures to use when training. The default is resnet18, resnet34, and resnet50
    ...
    if not training_csv_file.exists:
        raise LookupError(f'Could not find training split csv in {TRAIN_VALIDATE_SPLIT_DIR}. Run '
                          f'train_validate_split.py with the specified regions, training ratio, and tile size to create it')

    validate_csv_file = ValidateSplit(regions=regions, ratio=training_ratio, tile_size=tile_size)
